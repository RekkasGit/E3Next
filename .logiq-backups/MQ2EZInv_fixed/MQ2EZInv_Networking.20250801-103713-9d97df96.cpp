...
bool E3NextDirectNetworking::PublishInventory(const InventoryData& inventory)
{
    ...
    try {
        ...
        std::vector<uint8_t> jsonDataVec = inventory.Serialize();
        
        if (jsonDataVec.empty()) {
            WriteChatf("[MQ2EZInv] Failed to serialize inventory data");
            return false;
        }
        
        // Convert vector<uint8_t> to string for logging and pipe transmission
        std::string jsonData(jsonDataVec.begin(), jsonDataVec.end());
