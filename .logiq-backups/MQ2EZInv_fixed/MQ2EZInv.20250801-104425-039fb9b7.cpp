...
void EZInvE3StatusCommand(PlayerClient* pChar, const char* szLine) {
	...
	if (args.empty() || args == "status") {
		...
		// Check if E3Next pipe listener is available
		if (E3DirectNetwork::IsAvailable()) {
			WriteChatf("[MQ2EZInv] ✅ Direct Network: ACTIVE - Named pipes connected to E3Next");
		} else {
			WriteChatf("[MQ2EZInv] ⚠️  Direct Network: INACTIVE - E3Next pipe listener not running");
		}
...
PLUGIN_API void InitializePlugin() {
	...
	AddCommand("/ezinv", EZInvCommand); // Main command (already added above, but just to make sure)
