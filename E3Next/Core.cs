	...
	static public class EventProcessor
	{
		...
		public static void Init()
		{
			if (!_isInit)
			{
				...
				System.Text.RegularExpressions.Regex filterRegex = new Regex(@" points of damage\.", RegexOptions.Compiled);
		...
		public static void RegisterEvent(string keyName, string pattern, Action<EventMatch> method)
		{
			...
			eventToAdd.regexs.Add(new System.Text.RegularExpressions.Regex(pattern, RegexOptions.Compiled));
			eventToAdd.method = method;
			eventToAdd.keyName = keyName;
					...
					unsafe
					{
						...
						while (data.Length > 0 && !needsToDoRefresh)
						{
							...
							if (SpawnsByID.TryGetValue(spawnid, out var spawn))
							{
								...
								spawn.Aggressive = agressive;
								spawn.Dead = dead;
								spawn.Heading = heading;
								spawn.Height = height;
								spawn.MasterID = master;
								spawn.Moving = moving;
								spawn.PctHps = pctHps;
								spawn.PetID = petid;
								spawn.Targetable = targetable;
								spawn.TargetOfTargetID = targetoftargetID;
								spawn.X = s_x;
								spawn.Y = s_y;
								spawn.Z = s_z;
								spawn.playerX = p_x;
						...
						if (!needsToDoRefresh)
						{
							//check if we need to delete anything from our spawn collection
							foreach (var spawn in _spawns)
							{
								if (spawn.isDirty == false)
								{
									//means we have a spawn in our collection that the refresh doesn't had so a mob was killed.
									//just remove it and continue on
									_refershSpawnIdsToDelete.Add(spawn.ID);
								}
							}
