...
namespace E3Core.Processors
{
	...
	public static class Basics
	{
		...
		public static void RegisterEvents()
		{
			...
			EventProcessor.RegisterCommand("/e3brace", (x) =>
			{

				bool requestingInfo = false;

				if (x.args.Contains("info"))
				{
					requestingInfo = true;
					x.args.Remove("info");
				}

				if (x.args.Count == 0 && requestingInfo)
				{
					E3.Bots.BroadcastCommandToGroup($"/e3brace all info", x);

				}
				else if (x.args.Count == 0)
				{
					E3.Bots.BroadcastCommandToGroup($"/e3brace all", x);

				}

				foreach (var s in E3.CharacterSettings.BraceSpells)
				{
					...
					if (x.args.Count == 0)
					{
						if (requestingInfo)
						{
							string reportEntry = Create_E3BurnReportEntry(s);
							E3.Bots.Broadcast(reportEntry);
						}
						else
						{
							...
							{
								E3.Bots.Broadcast($"\agBracing for impact with \am{s.CastName}\ag!");
								Casting.Cast(0, s);
							}
						}
					}
			...
			{
				//temp fix to deal with a person being an ass
				if (true)
				{
					...
					if (x.match.Groups.Count > 1)
					{
						...
						if (E3.Bots.IsMyBot(person) || e3util.InMyGuild(person) || e3util.InMyRaid(person))
						{
							MQ.Cmd("/invite");
							MQ.Delay(300);
						}
						...
						{
							E3.Bots.Broadcast($"{person} tried to invite me to group, but not in my bot network/guild/raid, ignoring");
						}
					}
				}
				else
			...
			{
				//quick fix to deal with a person being an arse
				if (true)
				{
					...
					{
						string person = x.match.Groups[1].Value;
						if (e3util.InMyGuild(person) || E3.Bots.IsMyBot(person))
						{
							MQ.Delay(500);
							MQ.Cmd("/raidaccept");
						}
						...
						{
							E3.Bots.Broadcast($"{person} tried to invite me to raid, but not in my guild/bot network, ignoring");
						}
					}
				}
				else
		...
		public static void CheckFood()
		{
			...
			using (_log.Trace())
			{
				...
				if (foundFood && hungerLevel < 3400 && foundFoodCount > 0)
				{
					MQ.Cmd($"/useitem \"{toEat}\"");

					if (foundFoodCount < _notifyFoodDrinkAmount && foundFoodCount > 0)
					{
				...
				if (foundDrink && thirstLevel < 3400 && foundDrinkCount > 0)
				{
					MQ.Cmd($"/useitem \"{toDrink}\"");

					if (foundDrinkCount < _notifyFoodDrinkAmount && foundDrinkCount > 0)
					{
